...
- Internal properties `_n` and `_c` are used pervasively:
  - `Node.name` getter/setter maps to `_n`
  - `RepTree.newNamedNode()` writes `_n`
...
- Names
  - When a node is created via a "named" creator, store the name as `name` (string). No remapping.
  - `Node.name` reads/writes `name` directly.
  - Path lookups and printing use `name` if present.

- Creation time
  - Stop auto-writing `_c`. Instead, support opt‑in enrichment functions.
  - Provide a default enrichment that sets `createdAt: new Date()` when nodes are created.
...
- Reactive binding
  - Do not remap fields by default; `bindNode` should pass keys through untouched.
...
class RepTree {
  addEnricher(fn: NodeEnricher): () => void; // returns unregister
}
```

- Execution point: after node creation (`create`).
...
- RepTree
  - `addEnricher(fn: NodeEnricher): () => void`
  - Stop implicit `_c` write in `newNode*`
  - `newNamedNode` writes `name` instead of `_n`
...
- `newNamedNode` now writes `name` (no `_n`).
...
- Should `bindNode` coerce `createdAt` to Date on read by default? Recommendation: yes, when the key is exactly `createdAt`, as a small convenience; or leave it to an optional remapping rule for strictness.

## Minimal Implementation Steps
- Implement `addEnricher` (post-create only) in `RepTree`.
- Add `withCreatedAt` helper and use it in README examples.
- Switch `newNamedNode` to write `name`; update `Node.name`, `getNodeByPath`, printing, sorting to use `name`.
- Do not remap keys in `bindNode` by default; keep optional support only when explicitly provided.
