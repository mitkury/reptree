...
| ------------------- | ---------------------------------------- | ------------------------------------------------------ |
| JavaScript Thin API | TypeScript + Zod                         | Sync, optimistic node objects, small cache, undo log |
...
pub struct RepTree {
    ...
    cache: lru::LruCache<String, EncodedNode>,
    lamport: u64,
...
impl RepTree {
    ...
    pub async fn apply_op(&mut self, op: NodeOperation) -> Result<OpId, ValidationError>;
    pub async fn get_node(&mut self, id: &str) -> Option<EncodedNode>;
    pub async fn get_children(&mut self, parent_id: &str, after: Option<i64>, limit: u32)
        -> Result<Vec<EncodedNode>>;
...
pub trait NodeStore: Send + Sync {
    async fn get_node(&self, id: &str) -> Result<Option<EncodedNode>>;
    async fn put_node(&self, v: EncodedNode) -> Result<()>;
...
Default backend: SQLite tables `nodes`, `move_ops`, `prop_ops` with `(parent_id, idx)` composite index for seek-pagination.
...
loop {
    ...
    node_store.put_nodes(state.dirty()).await?;
}
```
...
type JsToRustOperation =
  | { type: "createNode"; parentId: string; properties?: Record<string, any>; id?: string }
  | { type: "moveNode"; nodeId: string; newParentId: string }
  | { type: "deleteNode"; nodeId: string }
  | { type: "setProperty"; nodeId: string; key: string; value: any }
  | { type: "removeProperty"; nodeId: string; key: string };

// JS → Rust
interface JsOpMsg  { type: "operation"; cid: number; op: JsToRustOperation }
interface FetchMsg { type: "getNode" | "getChildren"; id: string; after?: number; limit?: number }
interface SubMsg   { type: "subscribeNode"; id: string }
interface UnsubMsg { type: "unsubscribeNode"; id: string }
...
interface NodeMsg { type: "nodeUpdate"; id: string; data: NodeData }
interface DeleteMsg { type: "nodeDeleted"; id: string }
type RustToJs = AckMsg | NackMsg | NodeMsg | DeleteMsg
```
...
interface RepTreeWasm {
  ...
  getNode(handle: number, id: string): string; // Returns node JSON
  getChildren(handle: number, parentId: string, after: number | null, limit: number): string;
  subscribeNode(handle: number, id: string): void;
  // Other methods
}
```
...
class ThinRepTree {
  ...
  private cache = new LRU<string, NodeData>({ max: 50000 });
  private subscriptions = new Set<string>();
  private observers = new Map<string, Set<(data: NodeData) => void>>();
  ...
  subscribeToNode(id: string) {
    if (!this.subscriptions.has(id)) {
      this.subscriptions.add(id);
      this.bridge.post({ type: "subscribeNode", id });
    }
    return this;
  }
...
class ThinNode {
  ...
  newChild(props = {}) {
    ...
    return new ThinNode(this.rt, childId);
  }
...
```typescript
function bind<T>(schema: z.ZodType<T>, node: ThinNode): T {
  return new Proxy({} as T, {
    get(_, k) {
      return node.get(k as string);
    },
    set(_, k, v) {
      schema.shape[k as string]?.parse(v);  // Type check
      node.set(k as string, v);
      return true;
    }
  });
}
...
impl RepTree {
    ...
    fn get_missing_ops(&self, their_state: &StateVector) -> Vec<NodeOperation> {
...
* **LRU Cache**: Default 50,000 nodes (~4 MB) in both JS and Rust layers
...
* **Subscription Model**: Only transfer data for nodes the UI is actually using
...
impl NodeStore for SqliteNodeStore {
    async fn get_node(&self, id: &str) -> Result<Option<EncodedNode>> {
        // Query: SELECT * FROM rt_nodes WHERE id = ?
    }

    async fn put_node(&self, node: EncodedNode) -> Result<()> {
        // Query: INSERT OR REPLACE INTO rt_nodes VALUES (...)
    }

    async fn get_children_page(&self, parent_id: &str, after: Option<i64>, limit: u32)
        -> Result<Vec<(String, i64)>> {
        // Query: SELECT id, idx FROM rt_nodes
...
class IndexedDBStore {
  ...
  async getNode(id: string): Promise<EncodedNode | null> {
    // IndexedDB get operation
  }

  async putNode(node: EncodedNode): Promise<void> {
...
2. **Phase 1** – Zod proxies, live subscriptions, optimistic undo
   ...
   - Subscription-based node updates
...
3. **Scalability**: Native storage adapters enable handling millions of nodes efficiently
4. **Developer Experience**:
   ...
   - Direct node state access simplifies the mental model
