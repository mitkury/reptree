...
Provide an **in‑RAM, CRDT‑friendly file system** for workspaces and applications that embed RepTree.
...
## 2 — Node schema in the root *FS tree*
...
| `mount`      | `treeId`                                    | Mount another entire RepTree under this node   |

Other metadata (name, icons, size, timestamps…) remain normal node properties.
...
```
fsNodeId              # dirs, mounts, blobs
fsNodeId:innerId      # nodes inside linked RepTree
```
...
2. On `file-tree` node → `openTree(treeId)` if needed.
...
| FS action         | Effect                                                               |
|-------------------|----------------------------------------------------------------------|
| Create dir        | `newNode(parentId)` with `type: "dir"`                             |
| Create file       | allocate `treeId` or upload blob, then node with `type: "file-*"`  |
| Move / rename     | simply `moveNode` or property update in FS tree                    |
| Delete            | call `deleteNode()` which uses RepTree's void node approach      |
| Copy file         | duplicate node and storage as needed                               |
| Cross‑file move   | copy‑then‑delete once both docs are loaded                           |
| Recover deleted   | retrieve from void node if within recovery window                  |
...
* Hybrid child‑storage (see `reptree-nodes-with-lots-of-children.md`) for huge dirs.
...
* Any node property can be a Yjs document (text, map, array, etc.).
...
* **Permissions**: Node-level permission model with inheritance from parent nodes.
* **Access Control**: Read/write/admin permissions per node, with default inheritance.
...
2. Add helper API on `Node` (`asFile()`, `asDir()`, …).
