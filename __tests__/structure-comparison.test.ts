...
function printTreeStructure(label: string, tree: RepTree) {
  console.log(`\n--- ${label} ---`);
  console.log(`Total nodes: ${tree.getAllNodes().length}`);
  console.log(`Root ID: ${tree.root?.id}`);

  const printNode = (node: any, depth: number = 0) => {
    const indent = '  '.repeat(depth);
    const props = Object.entries(node.properties || {})
      .map(([key, value]) => `${key}: ${value}`)
      .join(', ');

    console.log(`${indent}- ${node.id}: ${node.name || '(unnamed)'} ${props ? `(${props})` : ''}`);

    const children = tree.getChildrenIds(node.id).map(id => tree.getNode(id));
...
describe('RepTree Structure Comparison', () => {
  test('simple tree duplicate should have identical structure', () => {
    ...
    // Create some child nodes with properties
    ...
    // Additional verification of nodes count
    expect(duplicateTree.getAllNodes().length).toBe(originalTree.getAllNodes().length);
  });
  ...
  test('duplicate with one additional operation should have different structure', () => {
    ...
    const duplicateDocs = duplicateTree.getNode(docsFolder.id)!;
  ...
  test('trees with different root IDs can never have identical structures', () => {
    ...
    // Verify both trees have the same number of nodes
    expect(treeA.getAllNodes().length).toBe(treeB.getAllNodes().length);
    ...
    // The merged tree will have both root nodes and their children
    console.log('\n===== MERGED TREE =====');
    printTreeStructure('Merged Tree', mergedTree);
    console.log('\nMerged Tree string representation:');
    console.log(mergedTree.printTree());

    // Verify that the merged tree contains more nodes than the individual trees
    // since it has both trees' root nodes and their children
    expect(mergedTree.getAllNodes().length).toBeGreaterThan(treeA.getAllNodes().length);
    ...
    // We expect all trees to have the same number of nodes after syncing
    expect(treeA.getAllNodes().length).toBe(treeB.getAllNodes().length);
    expect(treeA.getAllNodes().length).toBe(mergedTree.getAllNodes().length);
