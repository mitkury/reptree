...
describe('RepTree Synchronization Methods Comparison', () => {
  test('should compare efficiency of all-ops vs. state-vector sync', () => {
    ...
    const allOpsNodes = allOpsResult.trees[0].getAllNodes().length;
    const vectorNodes = vectorResult.trees[0].getAllNodes().length;
    console.log(`\nTree sizes: All-Ops: ${allOpsNodes.toLocaleString()} nodes, Vector: ${vectorNodes.toLocaleString()} nodes`);
