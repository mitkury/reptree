...
import { type NodeOperation } from '../src/operations';
...
describe('RepTree Fuzzy Testing', () => {
  test('should synchronize correctly without state vectors', () => {
    ...
    // Final verification
    const totalNodes = trees[0].getAllNodes().length;
    const finalMem = process.memoryUsage();
    console.log(`Test complete with ${totalOperations} operations performed, resulting in ${totalNodes} nodes`);
    console.log(`Final memory: ${(finalMem.heapUsed / 1024 / 1024).toFixed(2)} MB`);

    expect(totalNodes).toBeGreaterThan(1);

    // Verify all trees have identical node counts
    const referenceNodeCount = trees[0].getAllNodes().length;
    for (let i = 1; i < treesCount; i++) {
      expect(trees[i].getAllNodes().length).toBe(referenceNodeCount);
    }
  });
   ...
   * causal dependencies (e.g., you need to create a node before moving it).
   *
  ...
  test('causal dependency test: should handle out-of-order operations with shuffled merges', () => {
    ...
    for (let round = 0; round < rounds; round++) {
      ...
      try {
        ...
        for (let i = 0; i < treesCount; i++) {
          console.log(`Tree ${i+1} node count: ${trees[i].getAllNodes().length}`);
        }
    ...
    for (let round = 0; round < rounds; round++) {
      ...
      try {
        ...
        for (let targetIndex = 0; targetIndex < treesCount; targetIndex++) {
          ...
          const peerOps: Array<NodeOperation[]> = [];
          ...
          const allInterleaved: NodeOperation[] = [];
